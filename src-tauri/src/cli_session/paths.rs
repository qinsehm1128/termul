...
mod tests {
    ...
    fn encodes_vendor_project_folders() {
        let cwd = PathBuf::from("/Users/qs/project/me/termul");
        assert_eq!(
            encode_claude_project_dir(&cwd),
            "-Users-qs-project-me-termul"
        );
        assert_eq!(
    ...
    fn scoped_roots_only_open_matching_folder() {
        ...
        let keep = tmp.path().join("-Users-qs-project-me-termul");
        let other = tmp.path().join("-Users-qs-other");
        fs::create_dir_all(&keep).unwrap();
        fs::create_dir_all(other).unwrap();
        let scope = [PathBuf::from("/Users/qs/project/me/termul")];
        let mut roots = Vec::new();
